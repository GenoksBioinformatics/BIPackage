...
def count_reads_and_bases(fastq_file):
    try:
        read_count = int(subprocess.check_output(
            f"zcat {fastq_file} | wc -l", shell=True, text=True).strip()) // 4
        

        first_read_length = int(subprocess.check_output(
            f"zcat {fastq_file} | sed -n '2p' | wc -c", shell=True, text=True).strip()) - 1
...
with ThreadPoolExecutor() as executor:
    ...
    for future in futures:
        ...
        results.append({"file": os.path.basename(filename), "reads": read_count, "bases": base_count})

df = pd.DataFrame(results)

print(df)

df.to_csv("/home/genwork2/03.Fastq_Output/R208genklimerged/R208genklimerged-stats.csv", index=False)
