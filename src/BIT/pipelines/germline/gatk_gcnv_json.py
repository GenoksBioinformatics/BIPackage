...
class GCNV:
    def __init__(self, input_json, output_dir, exome_loc, 
    genome_fasta, ref_dict, contig_pp, class_coherence_length, 
    cnv_coherence_length, min_contig_length, p_active, p_alt, 
    interval_psi_scale, sample_psi_scale):
    ...
    def get_input_string(self):
        ...
        self.clist = [os.path.join(self.output_dir,item) for item in os.listdir(self.output_dir) if item.endswith("_counts.tsv")] #1
    ...
    def model_segments(self):
        ...
        arguments = [(os.path.splitext(os.path.basename(bam_path))[0], os.path.join(self.output_dir,os.path.splitext(os.path.basename(bam_path))[0]), self.output_dir) for bam_path in self.bam_list]
...
def main():
    ...
    parser.add_argument("--class_coherence_length", type=str, default="10000", help="Class coherence length")
    parser.add_argument("--cnv_coherence_length", type=str, default="10000", help="CNV coherence length")
    parser.add_argument("--min_contig_length", type=str, default="46709983", help="Minimum contig length")
    parser.add_argument("--p_active", type=str, default="0.01", help = "Prior probability of treating an interval as CNV-active")
    parser.add_argument("--p_alt", type=str, default="0.000001", help = "Total prior probability of alternative copy-number states")
    parser.add_argument("--interval_psi_scale", type=str, default="0.001", help = "Typical scale of interval-specific unexplained variance")
    parser.add_argument("--sample_psi_scale", type=str, default="0.0001", help = "Typical scale of sample-specific correction to the unexplained variance")
    

    args = parser.parse_args()
    
    counts = GCNV(args.input_json, args.output_dir, args.exome_loc, args.genome_fasta, args.ref_dict, 
                  args.contig_pp, args.class_coherence_length, args.cnv_coherence_length, args.min_contig_length,
                  args.p_active, args.p_alt, args.interval_psi_scale, args.sample_psi_scale)
