        ...
        for folder in self.input_folder_paths:
            for root, _, files in os.walk(folder):
                for filename in files:
                    if name.lower().strip() in filename.lower().strip() and (filename.endswith("R1_001.fastq.gz") or filename.endswith("2.fq.gz")) and "_MF_R2" not in filename:
                        r1_paths.append(os.path.join(root, filename))
                    if name.lower().strip() in filename.lower().strip() and (filename.endswith("R2_001.fastq.gz") or filename.endswith("2.fq.gz")) and "_MF_R2" not in filename:
